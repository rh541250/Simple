...
static NSUInteger OverlayerViewTag = 998;

@interface SIMScreenShotTool ()
{
    UIView *m_overLayerView;
...
- (void)createOverlayerViewWithImage:(UIImage *)image
{
    ...
    m_overLayerView = [[UIView alloc]initWithFrame:CGRectMake(window.frame.size.width - 100, window.frame.size.height/2 - 90, 100, 180)];
    m_overLayerView.layer.cornerRadius = 2.0;
    m_overLayerView.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.7];
    ...
    [m_overLayerView addSubview:photoImageView];
    ...
    [m_overLayerView addSubview:adviceButton];
    ...
    [m_overLayerView addSubview:shareButton];
    
    m_overLayerView.tag = OverlayerViewTag;
    [window addSubview:m_overLayerView];
}

- (void)pushToImageViewEdit:(UIButton *)sender
{
    [m_overLayerView removeFromSuperview];
