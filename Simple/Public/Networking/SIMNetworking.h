...
//项目打包上线不会打印日志
#ifdef DEBUG
#define SIMAppLog(s, ...) NSLog( @"[%@ in line %d] ===============>%@", [[NSString stringWithUTF8String:__FILE__] lastPathComponent], __LINE__, [NSString stringWithFormat:(s), ##__VA_ARGS__] )
#else
#define SIMAppLog(s,...)
#endif
 ...
 */
typedef void (^SIMDownloadProgress)(int64_t bytesRead,
                                    int64_t totalBytesRead
                                    );


typedef SIMDownloadProgress SIMGetProgress;
typedef SIMDownloadProgress SIMPostProgress;
 ...
 */
typedef void (^SIMUploadProgress)(int64_t bytesWritten,
                                  int64_t totalBytesWritten
                                 );

typedef NS_ENUM(NSUInteger,SIMResponseType){
    kSIMResponseTypeJSON = 1,//默认的格式
    kSIMResponseTypeXML,
    kSIMResponseTypeData
};


typedef NS_ENUM(NSUInteger,SIMRequestType){
    kSIMRequestTypeJSON = 1,//默认的格式
    kSIMRequestTypePlainText//text/html格式
};

typedef NS_ENUM(NSUInteger,SIMNetworkStatus){
    kSIMNetworkStatusUnknow = -1,       //未知网络
    kSIMNetworkStatusNotReachable = 0,//网络未连接
    kSIMNetworkStatusReachableViaWWAN = 1,//数据网络
    kSIMNetworkStatusReachableViaWiFi = 2,//wifi网络
};

@class NSURLSessionTask;
typedef NSURLSessionTask SIMURLSessionTask;
typedef void (^SIMResponseSuccess)(id response);
typedef void (^SIMResponseFail)(NSError *error);


@interface SIMNetworking : NSObject
...
/**
 *
 *	取消某个请求。如果是要取消某个请求，最好是引用接口所返回来的SIMURLSessionTask对象，
